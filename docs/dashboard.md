...
- `phase2DashboardController.dispatch('create-manual-run', { input })` validates a selected `agentId` plus a single chat `Message`, derives the manual run title and summary from that message, posts a selected-agent `/ General task` run through Local API, and refreshes the Runs list from Local API storage. The returned status reflects the HostConnector execution bridge and is not synthesized by the UI.
...
`createRuntimeDashboardApp()` is the official Runtime Dashboard app wrapper. It lazily creates the Phase 2 dashboard runtime on `mount()`, mounts through `createPhase2DashboardDomBinder()`, and exposes `mount()`, `refresh()`, `unmount()`, `isMounted()`, and `getStatus()`. `apps/ui/runtime-dashboard.html` is the static browser entrypoint for this wrapper and can be served with `npm run serve:runtime-dashboard`; `docs/previews/runtime-dashboard-preview.html` remains the live preview. Both accept a bearer token/base URL, mount `createRuntimeDashboardApp()` after an explicit click, and keep Local API calls behind dashboard Load/Refresh/Write Run Summary controls. The runtime entrypoint holds no bearer token. It ships a `%%LONDI_LOCAL_API_TOKEN%%` placeholder that `scripts/serve-runtime-dashboard.mjs` replaces with the process token when serving that one response, marked `cache-control: no-store`. On `127.0.0.1`/`localhost` with an injected token the dashboard mounts automatically; otherwise the manual token/base-URL connection form is shown.

`npm run serve:runtime` starts the Local API service and static Runtime Dashboard UI together without adding a process-manager dependency. It uses `LONDI_AGENT_OS_LOCAL_API_TOKEN` when supplied and otherwise mints a 32-byte URL-safe token for that process only, validates it before startup, passes the same value to both children in their environment, prints only the UI/API URLs, and sanitizes child output. The token is never printed and never written to disk. Browser authentication needs no console, no localStorage, and no manual copying. It does not proxy Local API or weaken the UI → Local API → HostConnector boundary.


### Runtime Dashboard Run Console slice

The dashboard renders a `New Run` section above `Runs` as a small chat composer: an `Agent` selector, a recent-message area, one `Message` textarea, and a `Send` button. The UI derives the manual run title and summary from the message and sends it only through Local API. Local API/HostConnector may execute the Direct pipeline for Codex or Claude Code when a registered Git project and execution bridge are available; the UI itself never starts a subprocess.

Conversations are threaded per project and agent. The first message opens an agent session and its id is stored on the run as `sessionId`; later messages to the same project and agent are delivered with that id so the agent keeps prior turns in context. Conversation memory continues, but the workspace does not — every message runs in a fresh worktree cut from the project's target branch, so files an earlier turn wrote are not present in a later one. Codex supports this today; Claude Code does not.

Manual run records use the contract `{ id, type: "manual", action: "ask-{agentId}-general-task", agentId, title, prompt, summary, status, createdAt, intent, agentResponse?, sessionId?, execution?, error? }`, where status is `queued`, `running`, `succeeded`, or `failed`. Local API persists `queued` before execution and updates the same record from the real bridge result. Without a bridge it stays `queued`; writing the record never produces `succeeded`. Obsidian write-back remains a separate explicit action, and execution never merges automatically.
