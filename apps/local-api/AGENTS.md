...
- Manual Run Console records are stored through HostConnector in `data/runs/runs.json` with their selected `agentId` and `projectId`. Creation persists `queued` first; when an execution bridge is configured, HostConnector records the real Direct pipeline result (`running`, `succeeded`, or `failed`). Without a bridge the record remains `queued` and must never become a synthetic success. `queued` is a valid run status everywhere, including the dashboard model — a persisted `queued` record must never break Load or Refresh.
- Registered projects are persisted through HostConnector in `data/projects/projects.json` and hydrated on service start, so projects survive a runtime restart. Each record stores `id`, `name`, `rootPath`, and the detected default branch (`targetBranch`, `main`/`master` aware) used as the run target branch. The dashboard never asks for a branch.
- Refreshing runs must reconcile, not stall: when an attempt is no longer supervised (the usual cause is a service restart), the run moves to a terminal state carrying `execution.recovery.state = 'Recovery Required'` with `autoResume: false`. It must never be reported as succeeded.
