...
## 2026-07-29 — Codex conversations continue one agent session per project and agent

- **Decision:** A conversation is scoped to one project and one agent. The first message opens a Codex session; its id is persisted on the run, and every later message in that pair is delivered with `codex exec resume <session-id>` so prior turns are carried forward. This closes the follow-up recorded on 2026-07-28.
- **Sandbox:** `codex exec resume` accepts no `--sandbox` flag, so the mode is restated as `-c sandbox_mode=workspace-write`. Without it a resumed turn silently reverts to the read-only default and rejects every write — the same failure `--sandbox` was added to fix. `danger-full-access` and the approval-bypass flags remain unused.
- **Session ids are untrusted input:** ids are shape-checked by `sanitizeAgentSessionId` before being persisted or placed on a command line; anything unrecognizable is dropped and the conversation starts a new session.
- **Evidence:** Codex 0.145.0 prints `session id:` to **stderr** when not attached to a terminal, so both streams are searched — a first live run captured no id until this was corrected. Live three-turn thread on `smoke-test-repo`: turn 1 stored `019fae52-291e-71b3-b3e7-4e7254a68ae9`; turn 2 answered `ELEPHANT-9931` from turn 1; turn 3 resumed the same session under `code-change` and wrote `thread-proof.txt` containing that codeword, proving memory and write access both survive a resume. Covered by `node tests/codex-adapter.test.mjs`, `node tests/direct-manual-run-executor.test.mjs`, `node tests/host-connector.test.mjs`, and `node tests/phase2-runtime-contracts.test.mjs`.
- **Guardrail:** Each run still executes in its own isolated worktree and branch, and nothing is merged automatically.
- **Known limitation:** Conversation memory continues across turns, but the workspace does not. Every message runs in a fresh worktree cut from the project's target branch, so files written by an earlier turn are not present in a later one — the agent may recall work its current worktree does not contain. Carrying a workspace across a thread is a separate slice.
- **Follow-up:** Claude Code has no thread continuity; it was deliberately left unchanged. Its adapter also fails health on Windows because it spawns the CLI without shell resolution (`ENOENT`), unlike the Codex adapter — a pre-existing defect, tracked separately.
